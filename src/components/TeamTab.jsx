    ...
    >
      <Card style={{ padding: 14, opacity: m.active ? 1 : 0.5, cursor: onClick ? "pointer" : "default" }} onClick={onClick}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#f0f0f5", letterSpacing: -0.2 }}>{m.name}</div>
...
{vue === "orga" && (function() {
  ...
  function MemberTile({ m, onRemove, isLeader, accent }) {
    ...
    return (
      <div style={{ background: "rgba(255,255,255,0.07)", borderRadius: 14, padding: isLeader ? "14px 16px" : "10px 14px", boxShadow: "0 4px 16px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.08)", display: "flex", alignItems: "center", gap: 12, position: "relative", borderLeft: isLeader ? "3px solid " + accent : "none", minWidth: isLeader ? 200 : 170 }}>
        <Avatar name={m.name} role={m.role} size={isLeader ? 46 : 38} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: isLeader ? 14 : 13, fontWeight: 600, color: "#f0f0f5", letterSpacing: -0.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.name}</div>
  ...
  function PickerModal({ gid, available, onClose }) {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.50)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
        <div style={{ background: "rgba(30,25,50,0.95)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderRadius: 20, padding: 24, width: 360, maxHeight: "70vh", display: "flex", flexDirection: "column", gap: 14, boxShadow: "0 20px 60px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.08)" }} onClick={function(e) { e.stopPropagation(); }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#f0f0f5" }}>Ajouter un membre</div>
          <input autoFocus value={search} onChange={function(e) { setSearch(e.target.value); }} placeholder="Rechercher..." style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.08)", fontSize: 13, outline: "none", background: "rgba(255,255,255,0.07)", color: "#f0f0f5" }} />
          <div style={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            {filtered.map(function(m) {
              var ops = Array.isArray(m.operators) ? m.operators : [m.operator].filter(Boolean);
              return (
                <div key={m.id} onClick={function() { addMember(gid, m.id); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 12, cursor: "pointer", background: "rgba(255,255,255,0.05)", transition: "background 0.1s" }}
                  onMouseEnter={function(e) { e.currentTarget.style.background = "rgba(255,255,255,0.10)"; }}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <span style={{ fontSize: 13, color: "rgba(255,255,255,0.55)" }}>{team.length} membres · {groups.length} équipe{groups.length !== 1 ? "s" : ""}{unassigned.length > 0 ? " · " + unassigned.length + " sans équipe" : ""}</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {groups.map(function(g, gi) {
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: gi * 0.08, ease: "easeOut" }}
              style={{ background: "rgba(255,255,255,0.03)", borderRadius: 18, border: "1px solid rgba(255,255,255,0.08)", overflow: "hidden" }}
            >
              {/* Group header */}
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: accent + "08" }}>
                ...
                <input value={g.name} onChange={function(e) { renameGroup(g.id, e.target.value); }}
                  style={{ flex: 1, fontSize: 14, fontWeight: 700, color: "#f0f0f5", background: "transparent", border: "none", outline: "none", letterSpacing: -0.3 }} />
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", fontWeight: 500 }}>{members.length} membre{members.length !== 1 ? "s" : ""}</span>
              ...
              <div style={{ padding: "16px 18px", display: "flex", alignItems: "center", gap: 0, flexWrap: "wrap" }}>
                ...
                {rest.length > 0 && (
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: "rgba(255,255,255,0.35)", letterSpacing: 0.8, textTransform: "uppercase", display: "block", marginBottom: 6 }}>Commerciaux</span>
        ...
        {/* Unassigned */}
        {unassigned.length > 0 && (
          <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 18, border: "1px dashed rgba(255,255,255,0.15)", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 18px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: "rgba(255,255,255,0.35)", flex: 1 }}>Sans équipe</span>
            ...
            <div style={{ padding: "16px 18px", display: "flex", flexWrap: "wrap", gap: 8 }}>
              {unassigned.map(function(m) { return <MemberTile key={m.id} m={m} onRemove={null} isLeader={false} accent="rgba(255,255,255,0.35)" />; })}
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        {unresList.length === 0 && (
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", margin: 0 }}>Tous les codes sont attribués à un commercial.</p>
        )}
          ...
          return (
            <Card key={login} style={{ padding: "12px 16px", marginBottom: 8, borderLeft: "3px solid #FF3B30" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.55)" }}>{ctrs.length} contrat{ctrs.length > 1 ? "s" : ""}</span>
                <span style={{ fontSize: 11, color: "rgba(255,255,255,0.35)" }}>dernier : {ctrs[ctrs.length - 1].date || "—"}</span>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          ...
          <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#f0f0f5" }}>Codes par commercial</h3>
        </div>
            ...
            return (
              <Card key={m.id} style={{ padding: "10px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#f0f0f5", minWidth: 150 }}>{m.name}</span>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flex: 1, alignItems: "center" }}>
                    ...
                    {codes.length === 0 && <span style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", fontStyle: "italic" }}>Aucun code attribué</span>}
                  </div>
                  ...
                  {vstAddOpen === m.id && <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <input
                      ...
                      style={{ padding: "4px 10px", borderRadius: 8, border: "1px solid #0071E340", fontSize: 12, width: 140, fontFamily: "monospace", outline: "none", background: "rgba(0,113,227,0.08)", color: "#f0f0f5" }}
                    />
...
<div><label style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.55)", display: "block", marginBottom: 4 }}>Nom</label><Inp value={f.name} onChange={function(v) { setF(Object.assign({}, f, { name: v })); }} placeholder="Nom" /></div>
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
<div><label style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.55)", display: "block", marginBottom: 4 }}>Role</label><Sel value={f.role} onChange={function(v) { setF(Object.assign({}, f, { role: v })); }} options={ROLES} style={{ width: "100%" }} /></div>
<div><label style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.55)", display: "block", marginBottom: 4 }}>Opérateurs</label><div style={{ display: "flex", gap: 12, marginTop: 4 }}>{OPERATORS.map(function(op) { var checked = (f.operators || []).indexOf(op) >= 0; return <label key={op} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, cursor: "pointer" }}><input type="checkbox" checked={checked} onChange={function(e) { var ops = (f.operators || []).filter(function(x) { return x !== op; }); if (e.target.checked) ops = ops.concat(op); setF(Object.assign({}, f, { operators: ops })); }} /><Badge color={OP_COLORS[op]}>{op}</Badge></label>; })}</div></div>
</div>
...
<div>
  <label style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.55)", display: "block", marginBottom: 6 }}>Codes VST</label>
  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
    ...
    {(f.vstCodes || []).length === 0 && <span style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", fontStyle: "italic" }}>Aucun code</span>}
  </div>
  <div style={{ display: "flex", gap: 6 }}>
    <input
      value={fVstInput}
      ...
      placeholder="vst-xxx + Entrée"
      style={{ flex: 1, padding: "6px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", fontSize: 12, fontFamily: "monospace", outline: "none", background: "rgba(255,255,255,0.07)", color: "#f0f0f5" }}
    />
    <Btn s="sm" v="secondary" onClick={function() {
      if (fVstInput.trim()) {
        var code = fVstInput.trim().toLowerCase();
...
{(f.vstCodes || []).length > 0 && (
<div>
  <label style={{ fontSize: 12, fontWeight: 600, color: "rgba(255,255,255,0.55)", display: "block", marginBottom: 6 }}>Codes temporaires prêtés à</label>
    ...
    {(f.lentCodes || []).map(function(lc) {
      ...
      return (
        <div key={lc.code + lc.borrowerId} style={{ display: "flex", alignItems: "center", gap: 8, background: "#FF9F0A0D", border: "1px solid #FF9F0A30", borderRadius: 8, padding: "5px 10px" }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: "#f0f0f5", flex: 1 }}>{borrower ? borrower.name : "?"}</span>
          <button type="button" onClick={function() { setF(Object.assign({}, f, { lentCodes: (f.lentCodes || []).filter(function(x) { return !(x.code === lc.code && x.borrowerId === lc.borrowerId); }) })); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.35)", fontSize: 14, padding: 0 }}>×</button>
        </div>
      );
    })}
    {(f.lentCodes || []).length === 0 && <span style={{ fontSize: 12, color: "rgba(255,255,255,0.35)", fontStyle: "italic" }}>Aucun prêt actif</span>}
  </div>
