...
var [vstInputs, setVstInputs] = useState({});
var [vstAddOpen, setVstAddOpen] = useState(null);
var [fVstInput, setFVstInput] = useState("");
...
function MemberCard({ m, onClick, animDelay }) {
  var borrowed = borrowerMap[m.id] || [];
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, delay: animDelay || 0, ease: "easeOut" }}
    >
      <Card style={{ padding: 14, opacity: m.active ? 1 : 0.5, cursor: onClick ? "pointer" : "default" }} onClick={onClick}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "var(--lo-ink)", letterSpacing: -0.2 }}>{m.name}</div>
            <div style={{ display: "flex", gap: 4, marginTop: 3, flexWrap: "wrap", alignItems: "center" }}>
              ...
              {m.permis && <Badge color="var(--lo-accent)">Permis</Badge>}
              {m.voiture && <Badge color="#7C3AED">Voiture</Badge>}
              {!m.active && <Badge color="var(--lo-danger)">Inactif</Badge>}
              {borrowed.map(function(bc) { return <Badge key={bc.code} color="var(--lo-taupe)">Code de {bc.lenderName.split(' ')[0]}</Badge>; })}
            </div>
          </div>
        </div>
      </Card>
...
{vue === "liste" && ROLES.map(function(role, gi) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
        {members.map(function(m, i) { return <MemberCard key={m.id} m={m} onClick={function() { openEdit(m); }} animDelay={i * 0.04} />; })}
      </div>
    </motion.div>
  );
})}
...
{vue === "orga" && (function() {
  var GROUP_PALETTE = ["var(--lo-primary)","var(--lo-accent)","var(--lo-taupe)","var(--lo-accent)","#FF2D55","#5AC8FA","#FF6B35","#00B4D8","#06D6A0","#E63946"];
  ...
  function MemberTile({ m, onRemove, isLeader, accent }) {
    ...
    return (
      <div style={{ background: "rgba(255,253,247,0.78)", borderRadius: 14, padding: isLeader ? "14px 16px" : "10px 14px", boxShadow: "0 4px 16px rgba(0,0,0,0.3), 0 0 0 1px rgba(76,87,96,0.10)", display: "flex", alignItems: "center", gap: 12, position: "relative", borderLeft: isLeader ? "3px solid " + accent : "none", minWidth: isLeader ? 200 : 170 }}>
        <Avatar name={m.name} role={m.role} size={isLeader ? 46 : 38} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: isLeader ? 14 : 13, fontWeight: 600, color: "var(--lo-ink)", letterSpacing: -0.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.name}</div>
        ...
        {onRemove && (
          <button onClick={onRemove} style={{ position: "absolute", top: 5, right: 5, background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.25)", fontSize: 16, lineHeight: 1, padding: 2, borderRadius: 99, display: "flex", alignItems: "center" }} title="Retirer">×</button>
        )}
      </div>
    );
  }
  ...
  function PickerModal({ gid, available, onClose }) {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.50)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
        <div style={{ background: "rgba(30,25,50,0.95)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderRadius: 20, padding: 24, width: 360, maxHeight: "70vh", display: "flex", flexDirection: "column", gap: 14, boxShadow: "0 20px 60px rgba(0,0,0,0.4), 0 0 0 1px rgba(76,87,96,0.10)" }} onClick={function(e) { e.stopPropagation(); }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: "var(--lo-ink)" }}>Ajouter un membre</div>
          <input autoFocus value={search} onChange={function(e) { setSearch(e.target.value); }} placeholder="Rechercher..." style={{ padding: "8px 12px", borderRadius: 10, border: "1px solid rgba(76,87,96,0.10)", fontSize: 13, outline: "none", background: "rgba(255,253,247,0.78)", color: "var(--lo-ink)" }} />
          <div style={{ overflowY: "auto", display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            {filtered.map(function(m) {
              var ops = Array.isArray(m.operators) ? m.operators : [m.operator].filter(Boolean);
              return (
                <div key={m.id} onClick={function() { addMember(gid, m.id); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 12, cursor: "pointer", background: "rgba(76,87,96,0.07)", transition: "background 0.1s" }}
                  onMouseEnter={function(e) { e.currentTarget.style.background = "rgba(76,87,96,0.10)"; }}
                  onMouseLeave={function(e) { e.currentTarget.style.background = "rgba(76,87,96,0.07)"; }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <span style={{ fontSize: 13, color: "var(--lo-muted)" }}>{team.length} membres · {groups.length} équipe{groups.length !== 1 ? "s" : ""}{unassigned.length > 0 ? " · " + unassigned.length + " sans équipe" : ""}</span>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {groups.map(function(g, gi) {
          ...
          return (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: gi * 0.08, ease: "easeOut" }}
              style={{ background: "rgba(76,87,96,0.05)", borderRadius: 18, border: "1px solid rgba(76,87,96,0.10)", overflow: "hidden" }}
            >
              {/* Group header */}
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", borderBottom: "1px solid rgba(76,87,96,0.08)", background: accent + "08" }}>
                ...
                <input value={g.name} onChange={function(e) { renameGroup(g.id, e.target.value); }}
                  style={{ flex: 1, fontSize: 14, fontWeight: 700, color: "var(--lo-ink)", background: "transparent", border: "none", outline: "none", letterSpacing: -0.3 }} />
                <span style={{ fontSize: 12, color: "var(--lo-faint)", fontWeight: 500 }}>{members.length} membre{members.length !== 1 ? "s" : ""}</span>
                <button onClick={function() { deleteGroup(g.id); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.25)", fontSize: 18, lineHeight: 1, padding: "0 2px" }} title="Supprimer">×</button>
              </div>
              ...
              <div style={{ padding: "16px 18px", display: "flex", alignItems: "center", gap: 0, flexWrap: "wrap" }}>
                ...
                {rest.length > 0 && (
                  <div style={{ flex: 1 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: "var(--lo-faint)", letterSpacing: 0.8, textTransform: "uppercase", display: "block", marginBottom: 6 }}>Commerciaux</span>
        ...
        {/* Unassigned */}
        {unassigned.length > 0 && (
          <div style={{ background: "rgba(76,87,96,0.05)", borderRadius: 18, border: "1px dashed rgba(76,87,96,0.14)", overflow: "hidden" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 18px", borderBottom: "1px solid rgba(76,87,96,0.08)" }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 700, color: "var(--lo-faint)", flex: 1 }}>Sans équipe</span>
            ...
            <div style={{ padding: "16px 18px", display: "flex", flexWrap: "wrap", gap: 8 }}>
              {unassigned.map(function(m) { return <MemberTile key={m.id} m={m} onRemove={null} isLeader={false} accent="var(--lo-faint)" />; })}
            </div>
          </div>
        )}
      </div>
    ...
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          <div style={{ width: 4, height: 20, borderRadius: 2, background: "var(--lo-danger)" }} />
          <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "var(--lo-danger)" }}>
            Codes non attribués {unresList.length > 0 ? "(" + unresList.length + ")" : "— tout est résolu ✓"}
          </h3>
        </div>
        {unresList.length === 0 && (
          <p style={{ fontSize: 13, color: "var(--lo-faint)", margin: 0 }}>Tous les codes sont attribués à un commercial.</p>
        )}
        {unresList.map(function(login) {
          var ctrs = unresGroups[login];
          return (
            <Card key={login} style={{ padding: "12px 16px", marginBottom: 8, borderLeft: "3px solid var(--lo-danger)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                <code style={{ fontSize: 13, fontWeight: 700, color: "var(--lo-danger)", background: "rgba(102,99,91,0.10)", padding: "3px 8px", borderRadius: 6 }}>{login}</code>
                <span style={{ fontSize: 12, color: "var(--lo-muted)" }}>{ctrs.length} contrat{ctrs.length > 1 ? "s" : ""}</span>
                <span style={{ fontSize: 11, color: "var(--lo-faint)" }}>dernier : {ctrs[ctrs.length - 1].date || "—"}</span>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          <div style={{ width: 4, height: 20, borderRadius: 2, background: "var(--lo-primary)" }} />
          <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "var(--lo-ink)" }}>Codes par commercial</h3>
        </div>
            ...
            return (
              <Card key={m.id} style={{ padding: "10px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 600, color: "var(--lo-ink)", minWidth: 150 }}>{m.name}</span>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flex: 1, alignItems: "center" }}>
                    {codes.map(function(code) {
                      return (
                        <span key={code} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(76,87,96,0.10)", border: "1px solid rgba(76,87,96,0.30)", borderRadius: 8, padding: "2px 8px", fontSize: 12, fontWeight: 600, color: "var(--lo-primary)" }}>
                          {code}
                          <button onClick={function() { removeVstCodeFromMember(code, m.id); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(76,87,96,0.80)", fontSize: 14, lineHeight: 1, padding: 0, display: "flex", alignItems: "center" }}>×</button>
                        </span>
                      );
                    })}
                    {codes.length === 0 && <span style={{ fontSize: 12, color: "var(--lo-faint)", fontStyle: "italic" }}>Aucun code attribué</span>}
                  </div>
                  <button onClick={function(e) { e.stopPropagation(); setVstAddOpen(vstAddOpen === m.id ? null : m.id); var v = {}; v[m.id] = ""; setVstInputs(Object.assign({}, vstInputs, v)); }}
                    style={{ width: 28, height: 28, borderRadius: 8, border: "1px dashed rgba(76,87,96,0.60)", background: vstAddOpen === m.id ? "rgba(76,87,96,0.10)" : "transparent", cursor: "pointer", color: "var(--lo-primary)", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 300, flexShrink: 0 }}>
                    {vstAddOpen === m.id ? "×" : "+"}
                  </button>
                  {vstAddOpen === m.id && <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    <input
                      ...
                      style={{ padding: "4px 10px", borderRadius: 8, border: "1px solid rgba(76,87,96,0.40)", fontSize: 12, width: 140, fontFamily: "monospace", outline: "none", background: "rgba(76,87,96,0.10)", color: "var(--lo-ink)" }}
                    />
...
<div>
  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--lo-muted)", display: "block", marginBottom: 6 }}>Codes VST</label>
  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
    {(f.vstCodes || []).map(function(code) {
      return (
        <span key={code} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "rgba(76,87,96,0.10)", border: "1px solid rgba(76,87,96,0.30)", borderRadius: 8, padding: "2px 8px", fontSize: 12, fontWeight: 600, color: "var(--lo-primary)" }}>
          {code}
          <button type="button" onClick={function() { setF(Object.assign({}, f, { vstCodes: (f.vstCodes || []).filter(function(c) { return c !== code; }) })); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(76,87,96,0.80)", fontSize: 14, lineHeight: 1, padding: 0 }}>×</button>
        </span>
      );
    })}
    {(f.vstCodes || []).length === 0 && <span style={{ fontSize: 12, color: "var(--lo-faint)", fontStyle: "italic" }}>Aucun code</span>}
  </div>
  <div style={{ display: "flex", gap: 6 }}>
    <input
      value={fVstInput}
      ...
      placeholder="vst-xxx + Entrée"
      style={{ flex: 1, padding: "6px 10px", borderRadius: 8, border: "1px solid rgba(76,87,96,0.14)", fontSize: 12, fontFamily: "monospace", outline: "none", background: "rgba(255,253,247,0.78)", color: "var(--lo-ink)" }}
    />
    <Btn s="sm" v="secondary" onClick={function() {
      if (fVstInput.trim()) {
        var code = fVstInput.trim().toLowerCase();
...
{(f.vstCodes || []).length > 0 && (
<div>
  <label style={{ fontSize: 12, fontWeight: 600, color: "var(--lo-muted)", display: "block", marginBottom: 6 }}>Codes temporaires prêtés à</label>
  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 8 }}>
    {(f.lentCodes || []).map(function(lc) {
      var borrower = team.find(function(m) { return m.id === lc.borrowerId; });
      return (
        <div key={lc.code + lc.borrowerId} style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(165,158,140,0.05)", border: "1px solid rgba(165,158,140,0.30)", borderRadius: 8, padding: "5px 10px" }}>
          <code style={{ fontSize: 12, fontWeight: 700, color: "var(--lo-taupe)" }}>{lc.code}</code>
          <span style={{ fontSize: 12, color: "var(--lo-muted)" }}>→</span>
          <span style={{ fontSize: 12, fontWeight: 600, color: "var(--lo-ink)", flex: 1 }}>{borrower ? borrower.name : "?"}</span>
          <button type="button" onClick={function() { setF(Object.assign({}, f, { lentCodes: (f.lentCodes || []).filter(function(x) { return !(x.code === lc.code && x.borrowerId === lc.borrowerId); }) })); }} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--lo-faint)", fontSize: 14, padding: 0 }}>×</button>
        </div>
      );
    })}
    {(f.lentCodes || []).length === 0 && <span style={{ fontSize: 12, color: "var(--lo-faint)", fontStyle: "italic" }}>Aucun prêt actif</span>}
  </div>
