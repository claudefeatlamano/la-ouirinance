...
var cardFadeIn = { initial:{ opacity:0, y:18 }, animate:{ opacity:1, y:0 }, transition:{ duration:0.35, ease:"easeOut" } };
var staggerContainer = { animate:{ transition:{ staggerChildren:0.06 } } };
var staggerItem = { initial:{ opacity:0, y:12 }, animate:{ opacity:1, y:0 }, transition:{ duration:0.28, ease:"easeOut" } };
...
function resolveAllVTA() {
  var updated = contracts.map(function(c) {
    ...
    var didResolve = resolved !== c.commercial;
    return Object.assign({}, c, { commercial: resolved, vtaResolved: didResolve ? true : c.vtaResolved });
  });
  saveContracts(updated);
}
...
function CList(list) {
  return <Card style={{ padding:0, overflow:"hidden" }}><motion.div variants={staggerContainer} initial="initial" animate="animate">{list.map(function(c,i){ return CRow(c,i); })}</motion.div></Card>;
}
    ...
    <div>
      ...
      <motion.div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(155px, 1fr))", gap:12 }} variants={staggerContainer} initial="initial" animate="animate">
        {comStatsRC.map(function(cs) {
          ...
          return (
            <motion.div key={cs.name} variants={staggerItem}>
              <Card onClick={function(){ setSelectedCom(cs.name); setComFrom(""); setComTo(""); }} style={{ padding:16, cursor:"pointer", textAlign:"center" }}>
...
<div>
  {/* 4 summary cards */}
  <motion.div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))", gap:14, marginBottom:20 }} variants={staggerContainer} initial="initial" animate="animate">
    {/* Aujourd'hui */}
